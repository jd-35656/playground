...
"""Generate CLI documentation from command tree YAML."""
...
# Configuration
CLI_TREE_PATH = Path(__file__).parent.parent / "docs" / "templates" / "cli.yaml"
OUTPUT_MD_PATH = "reference/cli.md"


def parse_cli_tree(cli_tree_path: Path) -> list[str]:
    """Parse CLI tree YAML and return flattened list of commands."""
    with open(cli_tree_path) as f:
        cli_tree: dict[str, Any] = yaml.safe_load(f)
...
def generate_cli_docs(cli_tree_path: Path) -> str:
    """Generate complete CLI documentation."""
    commands = parse_cli_tree(cli_tree_path)
...
def main() -> None:
    ...
    content = generate_cli_docs(CLI_TREE_PATH)
    write_docs(content)
