...
sync-local: clean-all
	@if [ -z "$(PATH)" ]; then \
		echo "Error: You must specify PATH variable, e.g. `make sync-local PATH=../my-repo`"; \
		exit 1; \
	fi
	...
	@echo "Syncing from local PATH: $(PATH)"

	@echo "Copying files (excluding .git and Makefile) to current directory..."
	@rsync -av --exclude='.git' --exclude='Makefile' $(PATH)/ ./

	@if [ -f "$(PATH)/Makefile" ]; then \
		cp "$(PATH)/Makefile" "./Makefile-source"; \
		echo "Makefile from cloned repo renamed to Makefile-source."; \
	else \
	...
	@echo "Adding, committing, and pushing changes..."
	@git add .
	@git commit -m "Local sync from $(PATH)" || echo "No changes to commit."
	@git push origin main --force
...
help:
	...
	@echo "  sync-local PATH=\"/path/to/local/dir\" - Sync current repo files from a local path."
